    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::InconsistentPaymentStatus => {
                write!(
                    formatter,
                    "paid status must match the payment received flag"
                )
            }
            Self::InconsistentPaymentDate => {
                write!(
                    formatter,
                    "payment received date must match the payment received flag"
                )
            }
            Self::DueDateBeforeCreation => write!(
                formatter,
                "due date cannot be before the document creation date"
            ),
...
impl<'connection> Repository<'connection> {
    ...
    pub fn has_cross_kind_id_collision(&self, id: &str) -> RepositoryResult<bool> {
        let record_kind: Option<String> = self
            .connection
            .query_row(
    ...
    pub fn list_clients(&self) -> RepositoryResult<Vec<StoredRecord<Client>>> {
        self.list_records(
            "client",
            RecordKind::Client,
            "ORDER BY json_extract(data_json, '$.name') COLLATE NOCASE",
        )
    }
    ...
    fn validate_document(&self, record: &StoredRecord<Document>) -> RepositoryResult<()> {
        ...
        if record
            ...
            .due_date
            .as_deref()
            .is_some_and(|due_date| due_date < &record.created_at[..10])
        {
...
fn calculate_totals(document: &Document) -> (i64, i64, i64) {
    let (subtotal_cents, tax_amount_cents) =
        document
            .line_items
            .iter()
            .fold((0, 0), |(subtotal, tax), item| {
                let line_total = (item.quantity_milliunits * item.unit_price_cents + 500) / 1_000
                    - item.discount_cents;
                let line_tax =
                    (line_total * i64::from(item.tax_rate_basis_points) + 5_000) / 10_000;
...
fn record_from_row<T: DeserializeOwned>(
    row: &Row<'_>,
    record_kind: RecordKind,
) -> rusqlite::Result<StoredRecord<T>> {
...
mod tests {
    ...
    fn rejects_totals_that_do_not_match_line_items() {
        ...
        let error = repository
            .create_document(&record)
            .expect_err("reject invalid totals");
    ...
    fn rejects_cross_kind_client_id_collision_without_replacement() {
        ...
        let error = repository
            .upsert_client(&client, false)
            .expect_err("reject cross-kind collision");
