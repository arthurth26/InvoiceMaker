...
const BACKUP_DIRECTORY_FILE: &str = "backup-directory.txt";
...
pub fn open_and_migrate(app: &AppHandle) -> Result<Connection, DatabaseError> {
    let database_directory = app
        .path()
        .app_local_data_dir()
        .map_err(DatabaseError::AppDataDirectory)?;
    open_and_migrate_at(&database_directory)
}

pub fn backup_directory(database_directory: &Path) -> Result<PathBuf, DatabaseError> {
    let location_file = database_directory.join(BACKUP_DIRECTORY_FILE);
    match fs::read_to_string(location_file) {
        Ok(location) => {
            let location = location.trim();
            if location.is_empty() {
                Ok(database_directory.to_path_buf())
            } else {
        ...
        Err(error) => Err(DatabaseError::FileSystem(error)),
    }
}

pub fn set_backup_directory(
    database_directory: &Path,
    backup_directory: &Path,
) -> Result<(), DatabaseError> {
    fs::create_dir_all(backup_directory).map_err(DatabaseError::FileSystem)?;
    fs::write(
        database_directory.join(BACKUP_DIRECTORY_FILE),
        backup_directory.to_string_lossy().as_bytes(),
    )
    .map_err(DatabaseError::FileSystem)
}
...
pub fn create_replacement_backup(
    connection: &Connection,
    database_directory: &Path,
    ...
    create_backup(
        connection,
        database_directory,
        "invoicemaker-before-replacement",
    )
}

pub fn create_deletion_backup(
    connection: &Connection,
    database_directory: &Path,
) -> Result<PathBuf, DatabaseError> {
    create_backup(
...
pub fn store_proof(
    ...
    let filename = source.file_name().ok_or_else(|| {
        DatabaseError::FileSystem(std::io::Error::new(
...
pub fn open_proof(relative_path: &str, database_directory: &Path) -> Result<(), DatabaseError> {
    tauri_plugin_opener::open_path(
        database_directory.join(relative_path),
        Option::<String>::None,
    )
...
mod tests {
    ...
    use super::{
        backup_directory, create_manual_backup, open_and_migrate_at, save_pdf, set_backup_directory,
    };

    #[test]
    fn opens_and_migrates_the_app_database() {
        let directory =
            std::env::temp_dir().join(format!("invoicemaker-db-test-{}", std::process::id()));
    ...
    fn creates_a_consistent_standalone_database_backup() {
        let directory =
            std::env::temp_dir().join(format!("invoicemaker-backup-test-{}", std::process::id()));
        let _ = fs::remove_dir_all(&directory);
    ...
    #[test]
    fn persists_the_automatic_backup_directory() {
        let directory = std::env::temp_dir().join(format!(
            "invoicemaker-backup-location-test-{}",
            std::process::id()
        ));
        let _ = fs::remove_dir_all(&directory);
        fs::create_dir_all(&directory).expect("create test database directory");
        let selected_directory = directory.join("automatic-backups");

        set_backup_directory(&directory, &selected_directory).expect("save backup directory");

        assert_eq!(
            backup_directory(&directory).expect("read backup directory"),
            selected_directory
        );
        fs::remove_dir_all(directory).expect("remove test database directory");
    }
    ...
    fn writes_pdf_contents_to_the_selected_destination() {
        let directory =
            std::env::temp_dir().join(format!("invoicemaker-pdf-test-{}", std::process::id()));
